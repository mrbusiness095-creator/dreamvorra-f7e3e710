...
export function markPaid(amount = 15000) { const account = getAccount(); if (!account) return null; const updated = { ...account, paid: true, paymentPending: false, balance: account.paid ? account.balance : Math.max(account.balance, amount) }; saveAccount(updated); return updated; }
